...
import 'component_sign_d_t_trash_model.dart';
export 'component_sign_d_t_trash_model.dart';

class ComponentSignDTTrashWidget extends StatefulWidget {
  const ComponentSignDTTrashWidget({
    super.key,
    int? currentTabbar,
  ...
  @override
  State<ComponentSignDTTrashWidget> createState() =>
      _ComponentSignDTTrashWidgetState();
}

class _ComponentSignDTTrashWidgetState
    extends State<ComponentSignDTTrashWidget> {
  late ComponentSignDTTrashModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => ComponentSignDTTrashModel());

    // On component load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      logFirebaseEvent('COMPONENT_SIGN_D_T_TRASH_ComponentSign_D');
      _model.queryTrashFile = await FolderTable().queryRows(
        queryFn: (q) => q
            .eqOrNull(
              'type',
              'file',
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Builder(
        builder: (context) {
          final messageData = _model.queryTrashFile?.toList() ?? [];
                ...
                label: DefaultTextStyle.merge(
                  ...
                  child: Builder(
                    builder: (context) => FlutterFlowIconButton(
                      ...
                      onPressed: () async {
                        logFirebaseEvent(
                            'COMPONENT_SIGN_D_T_TRASH_more_vert_outli');
                  ...
                  child: Align(
                    ...
                    child: Builder(
                      builder: (context) => InkWell(
                        ...
                        onTap: () async {
                          logFirebaseEvent(
                              'COMPONENT_SIGN_D_T_TRASH_Icon_c66z9vx4_O');
                          await showAlignedDialog(
                            barrierColor: Colors.transparent,
                            context: context,
                            isGlobal: false,
                  ...
                  child: Row(
                    ...
                    children: [
                      if (messageDataItem.isRead ?? true)
                        Builder(
                          builder: (context) => FlutterFlowIconButton(
                            ...
                            onPressed: () async {
                              logFirebaseEvent(
                                  'COMPONENT_SIGN_D_T_TRASH_timeline_outlin');
                              await showDialog(
                                barrierColor: Colors.transparent,
                                context: context,
                                builder: (dialogContext) {
                      ...
                      FlutterFlowIconButton(
                        ...
                        onPressed: () async {
                          logFirebaseEvent(
                              'COMPONENT_SIGN_D_T_TRASH_remove_red_eye_');
