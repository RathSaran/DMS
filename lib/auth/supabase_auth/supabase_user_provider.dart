...
class DMSSYSTEMAutomateTestSupabaseUser extends BaseAuthUser {
  DMSSYSTEMAutomateTestSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dMSSYSTEMAutomateTestSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DMSSYSTEMAutomateTestSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
